...
const DETAILS_URL = 'https://monitoring.solaredge.com/solaredge-web/p/systemData';
    ...
    function SolaredgeOptimizersNode(config) {
        ...
        node.on('input', async function (msg, send, done) {
            try {
                ...
                const token = await login(client, this.credentials.username, this.credentials.password);
    ...
    async function getTags(client, x_csrf_token, siteId, reporterIds) {
        ...
        await Promise.all(reporterIds.map(async id => {
            ...
            try {
                ...
                let tags_to_extract = ['reporterId', 'serialNumber', 'type', 'description', 'manufacturer'];
